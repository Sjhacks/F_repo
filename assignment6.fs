...
let rec expand num den radix = 
    Seq.delay (fun () ->
        let x = num*radix in
        cons ((x/den):int) (expand (x%den) den radix))
...
let integrateTerm (t: term):term =
    match t with
    | Term (c,e) -> let ex = e+1 in Term ((c/(float ex)),(ex))
(* Just 2 lines of code! *)

let rec integrateSeries sigma =
    Seq.delay (fun() -> 
        let head = first sigma in
            cons (integrateTerm head) (integrateSeries (rest sigma)))
(* Just 2 lines of code! *)

(* Examples for testing. *)
let sigma1 = Seq.initInfinite (fun i -> Term(1.0,i))
//let l = prefix 5 sigma1
let sigma2 = integrateSeries sigma1
//let b = prefix 5 sigma2

let rec expSeries = Seq.delay(fun () -> cons (Term (1.0,0)) (integrateSeries expSeries))
